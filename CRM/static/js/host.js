/**
 * Created by zengchunyun on 16/5/2.
 */
function Choices(ths) {
    ...
    switch ($(ths).attr("action")){
        ...
        case "del-col": //删除操作
            ...
            var del_len = getData['del'].length;  //获取字典对象长度,该对象为数组,所以这里获取这个对象的数组的个数
            ...
            swal(
                ...
                function(){
                    ...
                    if(del_len > 0){
                        AjaxSubmit(url, getData);  //将数据通过AJAX提交,参数,URL,提交的数据请求
                        for(var index in getData['del']){
                            $("#show-data-info").find(":checkbox").each(function () {
                                if(String($(this).val()) == String(getData['del'][index])){
    ...
    $.ajax({
        ...
        success:function (arg) {
            try {
                ...
                console.log(status, errorMsg, typeof errorMsg);
                if(typeof errorMsg == "string"){
                    ErrorMsg(errorMsg);  //对返回的消息进行操作处理
                }else {
                //     if(status){
                //         for(var key in errorMsg){
                //             $("[name="+ key + "]").siblings(":last").removeClass('hide').children(":last").text("该信息可用").css('color','yellow');
                //         }
                //         $("[add-row='true']").attr("data-valid","true");
                //     }else {
                //         for(var key in errorMsg){
                //             if(errorMsg[key].trim()){
                //                 $("[name="+key+"]").siblings(":last").removeClass('hide').children(":last").text(errorMsg[key]).css('color','red');
                //             }
                //         }
                //         $("[add-row='true']").attr("data-valid","false");
                //     }
                }
            }catch (errmsg){
    ...
    $td.each(function () {
        ...
        if(edit_status == "true"){
            if(!$(this).prop("edit-now")){
                ...
                }else if($(this).attr('edit-type')=='select_group'){
                    var group_html = $("[name='group_name']:first").clone();
                    group_html.removeClass('col-xs-3').val(before).attr("selected",true);
                    $(this).html(group_html);
                }else if($(this).attr('edit-type')=='select_status'){
                    var select_html = $("[name='status']:first").clone();
                    select_html.removeClass('col-xs-3').val(before).attr("selected",true);
